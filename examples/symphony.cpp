...
#define DATA_DIR "meta/symphony/bgr/"
#define SEG_DIR "meta/symphony/seg/val/"
#define MASK_DIR "meta/symphony/water_mask/val/"

#define MIN_CONTOUR_SIZE 50
#define MIN_EDGE_SIZE 40
...
int main(int argc, char* argv[]){
  ...
  char metaFn[256];
  if (surveyId == -1){
    sprintf(metaFn, "meta/symphony/surveys/%d/%d_c%d_db/pose.txt", sliceId, sliceId, camId);
  }
  else{
    sprintf(metaFn, "meta/symphony/surveys/%d/%d_c%d_%d/pose.txt", sliceId, sliceId, camId, surveyId);
  }
  printf("metaFn: %s\n", metaFn);
  ...
  for (size_t i=0; i<survey.size(); i++){
    ...
    for (size_t j=0; j<edges.size(); j++){
      ...
      // write des to file
      char outFn[512];
      sprintf(outFn, "res/symphony/features/des/%d/%s.txt", label, fn.c_str());
      faiss_des2file(outFn, file_mode, LF.des[j]);

      //// write kp to file
      //sprintf(outFn, "res/symphony/features/kp/%d/%s.txt", label, fn.c_str());
